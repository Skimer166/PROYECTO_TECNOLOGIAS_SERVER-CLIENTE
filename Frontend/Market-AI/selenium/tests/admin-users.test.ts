...
describe('Market-AI — Panel de Usuarios', () => {
  ...
  it('Debe mostrar las columnas de la tabla de usuarios', async () => {
    const colEstado = await driver.wait(
      until.elementLocated(By.xpath('//*[contains(.,"Estado")]')),
      10000
    );
    expect(await colEstado.isDisplayed()).toBe(true);

    const colRol = await driver.wait(
      until.elementLocated(By.xpath('//*[contains(.,"Rol")]')),
      10000
    );
    expect(await colRol.isDisplayed()).toBe(true);

    const colCorreo = await driver.wait(
      until.elementLocated(By.xpath('//*[contains(.,"Correo")]')),
      10000
    );
    await driver.sleep(PAUSE);
    expect(await colCorreo.isDisplayed()).toBe(true);
  });
